...
const TutorFee = () => {
  ...
  const gettingFeeDetails = async () => {
    let { data } = await feerepo.GetTutorFeeDetails(user.email);
          ...
          {fee.length > 0 ? (
            <>
              ...
              <div className="flex flex-col h-2/3 md:h-68  overflow-y-scroll gap-3 p-3 px-4 rounded-md mt-8 shadow-xl  shadow-primary bg-neutral">
                {fee.map((item, index) => (
                  <div
                    className="flex bg-white items-center justify-between w-full mx-auto p-2 rounded-md  text-secondary"
                    key={item.courseid}
                  >
                    <p className="w-20">{item.name}</p>
                    <p className="w-22">{item.coursename}</p>
                    <p className="w-12">{item.noOfLectures}</p>
