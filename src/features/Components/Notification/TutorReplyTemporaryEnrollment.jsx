...
const TutorReplyTemporaryEnrollment = ({ notification }) => {
  const { role, user } = useSelector((state) => state.auth);
  const dispatch = useDispatch();
  console.log("student temp request is", notification);
  ...
  const onAccept = async () => {
    const { data } = await tutorrepo.acceptStudentRequestTemp(
  ...
  const OnReject = async () => {
    const { data } = await tutorrepo.rejectStudentRequestTemp(
      notification.tutoremail,
      notification.studentemail,
      notification.courseid,
      notification.slot,
...
export default TutorReplyTemporaryEnrollment;
